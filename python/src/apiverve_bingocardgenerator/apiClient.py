...
class BingocardgeneratorAPIClientError(Exception):
...
class ValidationError(BingocardgeneratorAPIClientError):
...
class BingocardgeneratorAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {
        "size": {"type": "integer", "required": False, "min": 3, "max": 10, "default": 5},
        "freeSpace": {"type": "boolean", "required": False, "default": True}
    }
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the BingocardgeneratorAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises BingocardgeneratorAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/bingocardgenerator'
        self.headers = {
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise BingocardgeneratorAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[BingocardgeneratorAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns BingoCardGeneratorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises BingocardgeneratorAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise BingocardgeneratorAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise BingocardgeneratorAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise BingocardgeneratorAPIClientError(f"Request failed: {str(e)}")
