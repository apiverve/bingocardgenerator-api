...
class BingocardgeneratorValidationException implements Exception {
  final List<String> errors;

  BingocardgeneratorValidationException(this.errors);

  @override
  String toString() => 'BingocardgeneratorValidationException: ${errors.join("; ")}';
}
...
/// Bingo Card Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/bingocardgenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [size] - Card size (3x3 to 10x10) [min: 3, max: 10]
/// * [freeSpace] - Include free space in center
class BingocardgeneratorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'size': ValidationRule(type: 'integer', required: false, min: 3, max: 10),
    'freeSpace': ValidationRule(type: 'boolean', required: false),
  };

  BingocardgeneratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/bingocardgenerator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [BingocardgeneratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw BingocardgeneratorValidationException(errors);
    }
  }

  /// Execute a request to the Bingo Card Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<BingocardgeneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw BingocardgeneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return BingocardgeneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw BingocardgeneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw BingocardgeneratorException('Resource not found');
      } else {
        throw BingocardgeneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is BingocardgeneratorException) rethrow;
      throw BingocardgeneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Bingo Card Generator API client.
class BingocardgeneratorException implements Exception {
  final String message;

  BingocardgeneratorException(this.message);

  @override
  String toString() => 'BingocardgeneratorException: $message';
}
